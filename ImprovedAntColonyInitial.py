...
class ImprovedAntColony(TSP_Problem):
    ...
    def LenPath(self,path):
        ...
        return Lk
    def initialize(self,Tau,Tau0,Road,Probabilities,m):
        alpha=0.1
        rho=0.1
        beta=2
        phi=0.1
        ...
        for p in Cluster:
            ...
            for k in ant:
                tabu.append(p.copy())
                ex=random.choice(range(len(p)))
                tabu[k][0],tabu[k][ex]=tabu[k][ex],tabu[k][0]
            ...
            for i in np.arange(1,currentShortestPath.__len__()):
                ...
                Tau[x][y]+=alpha*dTau
                Tau[y][x]+=alpha*dTau
            plt.plot([self.Xs[i] for i in currentShortestPath],[self.Ys[i] for i in currentShortestPath],'-')
    ...
    def gothrough(self,NC):
        ...
        self.initialize(Tau,Tau0,Road,Probabilities,m)
